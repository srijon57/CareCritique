...
const UttoraHospitalsList = () => {
    ...
    const fetchHospitals = async () => {
        try {
            ...
            if (Array.isArray(response.data)) {
                const filteredHospitals = response.data.filter(hospital => hospital.HospitalArea?.toLowerCase() === "uttora");
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-black dark:text-white flex flex-col items-center p-6">
            <h2 className="text-4xl font-bold mb-8 text-cyan-800 dark:text-white">Hospitals in Uttora</h2>

            <div className="flex flex-col gap-6 w-full max-w-5xl">
                {hospitals.length === 0 ? (
                    <p className="text-gray-500 dark:text-gray-400 text-center">No hospitals found in Uttora.</p>
                ) : (
