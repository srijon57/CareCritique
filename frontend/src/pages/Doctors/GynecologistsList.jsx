...
const GynecologistsList = () => {
    ...
    useEffect(() => {
        ...
        fetchGynecologistsAndReviews();
    }, [location.search]);

    const fetchGynecologistsAndReviews = async () => {
        setLoading(true);
        try {
            ...
            if (Array.isArray(doctorsResponse.data)) {
                const gynecologistList = doctorsResponse.data
                    .filter(doctor => 
                        doctor.Specialty.toLowerCase() === "gynecologist"
                    )
                    .map(doctor => ({
                        id: doctor.DoctorID,
                ...
                const updatedGynecologists = await Promise.all(
                    gynecologistList.map(async (doctor) => {
                        try {
                ...
                setGynecologists(updatedGynecologists);
                setError(null);
            } else {
                setError("Failed to fetch gynecologists. Please try again later.");
            }
        } catch (error) {
            console.error(
                "Error fetching gynecologists:",
    ...
    const handleApplyFilters = () => {
        ...
        navigate(`/gynecologists?${params.toString()}`);
    };
    ...
    const filteredGynecologists = gynecologists.filter(doctor => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
                            ...
                            <div className="col-span-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        id="search"
                                        type="text"
                                        placeholder="Search gynecologists by name"
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-white">
                        {loading
                            ...
                            : `Showing ${filteredGynecologists.length} specialist${filteredGynecologists.length === 1 ? "" : "s"}`}
                    </h2>
                </div>
                ...
                {!loading && !error && filteredGynecologists.length === 0 && (
                ...
                <div className="space-y-6">
                    {filteredGynecologists.map(doctor => (
                        <div
                            key={doctor.id}
...
export default GynecologistsList;